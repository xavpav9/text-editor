...
class ScreenController:
    ...
    def blit_to_screen(self, text, line):
        self.screen.blit(self.font.render(text, True, WHITE), (self.padding_x / 2, self.padding_y + (self.letter_height + self.line_spacing) * line))

    def draw_text(self, text_before_cursor, text_after_cursor):
        chars_per_line = (self.screen.get_size()[0] - self.padding_x) // self.letter_width
        line = 0
        text_to_be_blitted = ""
        for word in text_before_cursor.split(" "):
            if len(text_to_be_blitted + word) > chars_per_line and text_to_be_blitted != "":
                self.blit_to_screen(text_to_be_blitted, line)
                text_to_be_blitted = ""
                line += 1
...
while running:
    ...
    screenController.draw_text(text.before_cursor, text.after_cursor)
